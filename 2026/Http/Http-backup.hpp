...
public:
    HttpResponse() : _httpversion("HTTP/1.1"), _blank_line(line_sep) {}
    ...
    std::string Serialize()
    {
        ...
        {
            SetHeader("Content-Lenth", std::to_string(_response_body.size()));
        }
        // 拼接报头
    ...
    bool ReadContent(const std::string &path)
    {
        ...
        if (!in.is_open())
        {
            LOG(LogLevel::WARING) << "文件打开失败" << path;
            return false;
        }
        ...
        if (size <= 0)
        {
            LOG(LogLevel::WARING) << "文件为空或异常" << path;
            return false;
        }
